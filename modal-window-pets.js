...
petsList2.addEventListener("click", (event) => {
  ...
  const object = petsArray.filter(
    (pet) => event.target.parentNode.lastElementChild.textContent === pet.name
  );
  ...
  <p class="modal-text">${pet.description}</p>
  <ul class="modal-list">
  <li class="modal-item"><b>Age: </b>${pet.age}</li>
  <li class="modal-item"><b>Inoculations: </b>${pet.inoculations}</li>
  <li class="modal-item"><b>Diseases: </b>${pet.diseases}</li>
  <li class="modal-item"><b>Parasites: </b>${pet.parasites}</li>
  </ul></div>`;
    })
    .join("");

  backdropRef.innerHTML = `<div class="modal">${markup}</div>`;

  backdropRef.classList.add("modal-open");

  buttonModalClose.classList.add("is-open");
});

buttonModalClose.addEventListener("click", () => {
  backdropRef.classList.remove("modal-open");
